...
impl<'a> Options<'a> {
    ...
    pub(crate) fn with_prefix_commands(mut self, prefix_commands: Option<&'a str>) -> Self {
        self.prefix_commands_with = prefix_commands;
        self
    }
        ...
        match self.execution_mode {
            Default => {
                for command in &self.commands {
                    ...
                    if let Some(prefix) = self.prefix_commands_with {
                        buffer_command.push_str(format!("{prefix} ").as_str());
                    }
...
"#;
        ...
        #[test]
        fn format_as_shell_script_with_prefix_command() {
            let commands = Commands {
                commands: vec![
                    Command {
                        lines: vec!["echo \"Line 1\""],
                    },
                    Command {
                        lines: vec!["echo \"Line 2\""],
                    },
                    Command {
                        lines: vec!["echo \"Line 3\""],
                    },
                ],
                execution_mode: Default,
